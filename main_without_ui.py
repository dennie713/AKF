...
import Cal, CFD, AKF, zero_phase_filter, KF_v2, LAE
...
if __name__ == "__main__":
    ...
    Pos_AKF_est, Vel_AKF_est, Acc_AKF_est, Q_acc, Q_vel, u_values = AKF.AKF_2(0.001, Pos, PosCmd, VelCmd, AccCmd)
    Pos_AKF_est_mouse, Vel_AKF_est_mouse, Acc_AKF_est_mouse, Q_acc, Q_vel, u_values = AKF.AKF_2(0.001, mouse_real_Pos, PosCmd, VelCmd, AccCmd)
    ...
    plt.plot(t, Vel_est, label="LKF Motor Velocity", linewidth=2)
    plt.title("Motor and Mouse Velocity Comparison")
    plt.xlabel("Time (sec)")
    plt.yticks(np.arange(-1000, 1000, 50))
    plt.plot(t, Vel_est_mouse_real, 'r', label="LKF Mouse Velocity", linewidth=1)
    plt.ylabel("LKF Mouse Velocity (IPS)")
    plt.legend(loc="lower center")
    plt.grid()
    ...
    plt.plot(t, Acc_est, label="LKF Motor Acceleration", linewidth=2)
    plt.title("Motor and Mouse Acceleration Comparison")
    plt.xlabel("Time (sec)")
    plt.yticks(np.arange(-100, 100, 10))
    plt.plot(t, Acc_est_mouse_real, 'r', label="LKF Mouse Acceleration", linewidth=1)
    ...
    # 位置比较图
    plt.figure(11)
    plt.plot(t, Pos_est, label="Motor Position", linewidth=3)
    plt.plot(t, Pos_est_mouse, label="Motor Position", linewidth=2)
